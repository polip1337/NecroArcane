...
export default class Proxy {
	 ...
	 */
	get id() {
		return "prox_" + (this.item ? this.item.id : "") + "_" + (this.target ? this.target.id : "");
	}
	...
	get repeat() {
		return (this._item && this._item.repeat) || false;
	}
	...
	maxed(addedValue = 0) {
		return !this.item || this.item.maxed(addedValue);
	}
	canUse(g) {
		return this._item && this._item.canUse(g);
	}
	canRun(g) {
		return this._item && this._item.canRun(g);
	}
