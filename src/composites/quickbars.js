...
export default class Quickbars {
	...
	next() {
		if (++this._index >= this.bars.length) this._index = 0;
		this._active = this._bars[this._index];
	}

	prev() {
		if (--this._index < 0) this._index = this._bars.length - 1;
		this._active = this._bars[this._index];
	}

	dataDeleted(it) {
		if (!it) return;
		for (let i = this.bars.length - 1; i >= 0; i--) this._bars[i].remove(it.id);
	}
