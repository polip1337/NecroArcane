...
export default class Enchanting extends Proxy {
	...
	percent() {
		return this._length ? Math.round((100 * this._exp) / this._length) : 0;
	}
	...
	set length(v) {
		this._length = new Stat(v);
	}

	canRun(g) {
		return !this.done && this._item && this._target && this._item.canAlter(this._target);
	}
	get done() {
